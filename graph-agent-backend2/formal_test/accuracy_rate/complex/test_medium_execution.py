# -*- coding: utf-8 -*-
"""
Agent Gremlin 执行准确率测试脚本（complex级别）- SSE版本

功能：
1. 从 ground_truth_answers.json 提取 complex 级别的测试用例
...
# 添加项目根目录到 Python 路径
# 当前文件: formal_test/accuracy_rate/complex/test_complex_execution.py
# 需要向上4层到达项目根目录
...
def load_complex_test_cases(file_path: str) -> List[Dict[str, Any]]:
    """加载 complex 级别的测试用例"""
    try:
        ...
        # 只筛选 complex 级别的测试用例
        complex_cases = [tc for tc in all_test_cases if tc.get("level") == "complex"]
        print(f"✅ 成功加载 {len(complex_cases)} 个 complex 级别测试用例（总共 {len(all_test_cases)} 个）")
...
def test_agent_gremlin_execution(test_cases: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    print("\n" + "="*80)
    print("开始执行 Agent Gremlin 查询测试（complex 级别 - SSE 版本）")
    print("="*80 + "\n")
...
def generate_report(results: Dict[str, Any], output_dir: str):
    ...
    # 生成 JSON 报告
    json_report_path = os.path.join(output_dir, "agent_gremlin_execution_results_complex_sse.json")
    with open(json_report_path, 'w', encoding='utf-8') as f:
        json.dump(results, f, ensure_ascii=False, indent=2)
    print(f"📄 JSON 报告已保存: {json_report_path}")
    
    # 生成文本摘要报告
    txt_report_path = os.path.join(output_dir, "summary_complex2.txt")
    with open(txt_report_path, 'w', encoding='utf-8') as f:
        f.write("="*80 + "\n")
        f.write("Agent Gremlin 查询执行准确率测试报告（complex 级别 - SSE 版本）\n")
        f.write("="*80 + "\n\n")
...
def main():
    """主函数"""
    print("="*80)
    print("Agent Gremlin 查询执行准确率测试（complex 级别 - SSE 版本）")
    print("="*80)
    ...
    test_cases = load_complex_test_cases(ground_truth_file)
    
    if not test_cases:
        print("❌ 没有可用的 complex 级别测试用例")
        sys.exit(1)
