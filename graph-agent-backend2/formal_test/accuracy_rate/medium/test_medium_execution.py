...
# 添加项目根目录到 Python 路径
# 当前文件: formal_test/accuracy_rate/medium/test_medium_execution.py
# 需要向上4层到达项目根目录
...
def generate_report(results: Dict[str, Any], output_dir: str):
    ...
    # 生成 JSON 报告
    json_report_path = os.path.join(output_dir, "agent_gremlin_execution_results_medium_sse.json")
    with open(json_report_path, 'w', encoding='utf-8') as f:
        json.dump(results, f, ensure_ascii=False, indent=2)
    print(f"📄 JSON 报告已保存: {json_report_path}")
    
    # 生成文本摘要报告
    txt_report_path = os.path.join(output_dir, "summary_medium2.txt")
